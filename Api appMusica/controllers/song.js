...
const prueba = (req, res) => {
  return res.status(200).send({
    status: "success",
    message: "Prueba de controlador de canción"
  });
}
...
const save = async (req, res) => {
  ...
  if (!params.album || !params.track || !params.name || !params.duration) {
  ...
  try {
    ...
    return res.status(500).send({ message: 'Error al guardar la canción', error: error.message });
  }
}
  ...
  try {
    ...
    if (!songUpdated) {
      ...
      return res.status(500).send({
        status: "error",
        message: "Error al guardar el archivo de audio de la canción",
      });
    }
